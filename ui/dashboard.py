...
import io, json, os, tempfile, warnings, zipfile
...
# ─────────────────────────────────────────────────────────────
# 🧠 MULTI‑AI PROVIDER (Anthropic · OpenAI · Gemini · Groq)
# ─────────────────────────────────────────────────────────────
class MultiAIProvider:
    ...
    def generate(self, prompt: str, max_tokens: int = 1000) -> str:
        try:
            ...
            return f"❌ AI Error ({self.provider}/{self.model}): {e}"

    @classmethod
    def get_available(cls) -> dict:
        return {k: v for k, v in cls.PROVIDERS.items() if os.getenv(v["env_key"])}

    @classmethod
    def check_key(cls, provider: str) -> bool:
        return bool(os.getenv(cls.PROVIDERS[provider]["env_key"]))


# ─────────────────────────────────────────────────────────────
# DB CONNECTION HELPERS
# ─────────────────────────────────────────────────────────────
def connect_sqlite(f) -> tuple[create_engine, str]:
    tmp = tempfile.NamedTemporaryFile(delete=False, suffix=".db")
    tmp.write(f.getbuffer())
    tmp.close()
    return create_engine(f"sqlite:///{tmp.name}"), tmp.name

def connect_csvs(files) -> tuple[create_engine, str, list[str]]:
    tmp = tempfile.NamedTemporaryFile(delete=False, suffix=".db")
    tmp.close()
    ...
    return eng, tmp.name, loaded

def connect_url(url: str):
    return create_engine(url), url

def test_engine(eng) -> bool:
    try:
...
def extract_schema(eng) -> dict:
    ...
    for tbl in insp.get_table_names():
        ...
        for fk in fks_raw:
            for c, rc in zip(fk.get("constrained_columns", []), fk.get("referred_columns", [])):
                fk_map[c] = {"ref_table": fk.get("referred_table", ""), "ref_col": rc}
            fk_list.append({
    ...
    # Heuristic FK for CSV/Olist style (customer_id → customers.id)
    ...
    for tbl_name, tbl in tables.items():
        for col in tbl["columns"]:
            ...
            for cand in [base, base + "s", base + "es"]:
                if cand in tbl_names and cand != tbl_name:
...
def sample_all(eng, schema: dict, n: int) -> dict[str, pd.DataFrame]:
    out = {}
    for tbl in schema["tables"]:
        try:
            with eng.connect() as c:
                out[tbl] = pd.read_sql(text(f'SELECT * FROM "{tbl}" LIMIT :n'), c, params={"n": n})
        except Exception:
...
def _is_datetime_str(s: pd.Series) -> bool:
    try:
        ...
        if sample.empty:
            return False
        hits = sum(1 for v in sample if any(p in v for p in ["-", "/", "T", ":"]) and len(v) >= 8)
...
def _profile_col(s: pd.Series) -> dict:
    ...
    try:
        ...
        if pd.api.types.is_numeric_dtype(s):
            ...
            if _is_datetime_str(clean):
                try:
                    ...
                    if not parsed.empty:
                        p.update(_dt_stats(parsed)); p["col_kind"] = "datetime_string"
                        return p
                except Exception:
                    pass
        ...
        return {**base, "col_kind": "error"}

def profile_quality(schema: dict, samples: dict) -> dict:
    out = {}
    for tbl, df in samples.items():
        if df is None or df.empty:
            out[tbl] = {"table_name": tbl, "sampled_rows": 0, "column_count": 0,
                        "overall_completeness": 0.0, "worst_columns": [], "columns": {}}
            continue

        cols_prof = {c: _profile_col(df[c]) for c in df.columns}
        vals = [v["completeness"] for v in cols_prof.values()]
        overall = round(float(np.mean(vals)), 4) if vals else 0.0
        worst = sorted(cols_prof.items(), key=lambda x: x[1].get("null_rate", 0), reverse=True)
        ...
        out[tbl] = {
            ...
            "worst_columns": [w[0] for w in worst[:3] if w[1].get("null_rate", 0) > 0],
            "columns": cols_prof,
        }
    return out
...
def render_er_chart(G: nx.DiGraph) -> go.Figure:
    if G.number_of_nodes() == 0:
        fig = go.Figure()
        fig.add_annotation(text="No tables found", showarrow=False, x=0.5, y=0.5)
        return fig
    ...
    fig.add_trace(go.Scatter(
        ...
        hovertext=[f"<b>{n}</b><br>Rows: {G.nodes[n].get('row_count',0):,}<br>Links: {G.degree(n)}"
                   for n in G.nodes()],
        hoverinfo="text",
    ))
...
class CausalRel:
    def __init__(self, ft, fc, tt, tc, method, strength, pval, direction="→", insight=""):
        self.from_table, self.from_col = ft, fc
        self.to_table, self.to_col = tt, tc
        self.method, self.strength, self.p_value = method, strength, pval
        self.direction, self.insight = direction, insight
    def to_dict(self):
        return {"from": f"{self.from_table}.{self.from_col}",
                "to": f"{self.to_table}.{self.to_col}",
                "method": self.method, "strength": round(float(self.strength), 4),
                "p_value": round(float(self.p_value), 4), "direction": self.direction,
...
# ─────────────────────────────────────────────────────────────
# HEALTH, CONTRACTS, AI NARRATOR
# ─────────────────────────────────────────────────────────────
class HealthScore:
    LEVELS = [(75, "Healthy", "✅", "#3dd68c"),
              (50, "At Risk", "⚠️", "#f5a623"),
              (0, "Critical", "🚨", "#f47067")]
    def __init__(self, name, score, bd, rec):
        self.table_name, self.score = name, score
        self.breakdown, self.recommendation = bd, rec
    ...
    def to_dict(self):
        return {"table": self.table_name, "score": self.score, "label": self.label,
                "icon": self.icon, "breakdown": self.breakdown,
...
def compute_health(schema: dict, quality: dict) -> dict[str, HealthScore]:
    ...
    for tbl, metrics in quality.items():
        ...
        out[tbl] = HealthScore(tbl, composite, bd, rec)
    return out
...
def generate_contracts(schema: dict, quality: dict) -> dict[str, str]:
    ...
    for tbl, metrics in quality.items():
        ...
        contract = {
            ...
            "generated_by": "DB Intelligence Agent — Causal Engine",
            "table": tbl,
            "sla": {
...
def generate_ai(schema, er_summary, quality, causal_rels, api_key, model) -> dict:
    ...
    if not api_key:
        ...
        ai["schema"] = (f"This database has {n} tables, {c} columns, and {r:,} rows. "
                        "Add an Anthropic/OpenAI/Groq key in the sidebar to enable AI summaries.")
        ai["causal"] = "Causal relationships found — add API key for business interpretation."
        ai["tables"] = {
            t: (f"BUSINESS PURPOSE: The '{t}' table stores {tb['row_count']:,} records. "
    ...
    # Use the selected provider from session (if any), otherwise fallback to Anthropic logic
    provider = st.session_state.get("ai_provider", "anthropic")
    ai_client = MultiAIProvider(provider=provider, model=model)

    tbl_sizes = {n: {"rows": t["row_count"], "cols": len(t["columns"])}
    ...
    prompt_schema = (
        ...
        f"Write for a business audience. No bullet points."
    )
    ai["schema"] = ai_client.generate(prompt_schema, max_tokens=400)
    ...
    if causal_rels:
        ...
        ai["causal"] = ai_client.generate(prompt_causal, max_tokens=300)
    else:
    ...
    for tbl_name, tbl in schema["tables"].items():
        ...
        ai["tables"][tbl_name] = ai_client.generate(prompt_tbl, max_tokens=500)
    return ai
...
# STREAMLIT UI
# ─────────────────────────────────────────────────────────────
st.set_page_config(page_title="DB Intelligence Agent — Causal Engine", layout="wide")

# ── Sidebar: AI Provider (only one, clean) ───────────────────
st.sidebar.markdown("## 🤖 Choose AI Provider")
available = MultiAIProvider.get_available()
if not available:
    st.sidebar.error("❌ No API keys found in .env!")
else:
    provider_options = {f"{info['icon']} {info['name']}": key for key, info in available.items()}
    selected_label = st.sidebar.selectbox("AI Provider", options=list(provider_options.keys()))
    ...
    st.sidebar.success(f"✅ {selected_label}\n\n📦 `{selected_model}`")
    st.session_state["ai_provider"] = selected_provider
    st.session_state["ai_model"] = selected_model
    if st.sidebar.button("🧪 Test AI Connection"):
        with st.spinner("Testing..."):
            ai_test = MultiAIProvider(provider=selected_provider, model=selected_model)
            res = ai_test.generate("Say hello in one line and tell your name.", max_tokens=50)
            st.sidebar.info(f"💬 {res}")

st.sidebar.divider()
st.sidebar.markdown("## 🧠 DB Intelligence Agent")
st.sidebar.caption("Schema · Quality · Causal AI · Governance")

# Upload controls
st.sidebar.subheader("📂 Upload Your Data")
mode = st.sidebar.radio("Input type", ["🗄️ SQLite File (.db)", "📊 CSV Files", "🔗 Database URL"],
...
if mode == "🗄️ SQLite File (.db)":
    f = st.sidebar.file_uploader("SQLite database", type=["db", "sqlite", "sqlite3"],
                                 label_visibility="collapsed")
    if f:
        try:
            ...
            if test_engine(engine):
                ...
                st.sidebar.error("❌ Cannot read file")
        except Exception as e:
            st.sidebar.error(f"❌ {e}")

elif mode == "📊 CSV Files":
    files = st.sidebar.file_uploader("CSV files (multiple)", type=["csv"],
                                     accept_multiple_files=True, label_visibility="collapsed")
    if files:
        try:
            engine, _, loaded = connect_csvs(files)
            if test_engine(engine) and loaded:
                st.sidebar.success(f"✅ Loaded {len(loaded)} tables")
                ready = True
            else:
                st.sidebar.error("❌ No tables loaded")
        except Exception as e:
            st.sidebar.error(f"❌ {e}")
else:
    url_in = st.sidebar.text_input("SQLAlchemy URL",
    ...
    if url_in:
        try:
            ...
            if test_engine(engine):
                ...
                st.sidebar.error("❌ Cannot connect")
        except Exception as e:
            st.sidebar.error(f"❌ {e}")

st.sidebar.divider()
st.sidebar.subheader("⚙️ Settings")
# Allow manual API key override (optional) — otherwise uses .env
api_key_in = st.sidebar.text_input("Override API Key (optional)", type="password",
                                   placeholder="sk-... or leave blank for .env")
model_in = st.session_state.get("ai_model", "claude-sonnet-4-20250514")
n_rows = st.sidebar.slider("Sample rows per table", 500, 50000, 5000, 500)

st.sidebar.divider()
run_btn = st.sidebar.button("🚀 Analyze", type="primary", use_container_width=True, disabled=not ready)
clear_btn = st.sidebar.button("🗑️ Clear", use_container_width=True)
if clear_btn:
...
def run_analysis(engine, api_key, model, n_rows):
    ...
    status = st.empty()
    def upd(p, msg):
        bar.progress(p); status.info(f"⏳ {msg}")

    upd(8, "Extracting schema — tables, columns, keys...")
    ...
    upd(58, "⭐ Causal Phase 1 — Discovering directional relationships...")
    ...
    ai = generate_ai(schema, er_summary, quality, causal_rels,
                     api_key or (os.getenv("ANTHROPIC_API_KEY") if st.session_state.get("ai_provider")=="anthropic" else None),
                     model)
    ...
    return {
        "schema": schema, "quality": quality, "G": G, "er_summary": er_summary,
        "causal_rels": causal_rels, "health_scores": health_scores,
        "contracts": contracts, "ai": ai, "md": md, "json_report": jr, "zip_bytes": zb,
    }
...
if run_btn and engine and ready:
    try:
        st.session_state["R"] = run_analysis(
            engine,
            api_key_in,
            st.session_state.get("ai_model", "claude-sonnet-4-20250514"),
            n_rows,
        )
    except Exception as e:
...
if "R" not in st.session_state:
    st.markdown("# 🧠 DB Intelligence Agent — Causal Intelligence Engine")
    st.markdown("Upload a database (SQLite/CSV/URL) in the sidebar → click **Analyze** → get full AI-powered analysis, ER diagram, and data contracts.")
    st.markdown("---")
    c1, c2, c3, c4 = st.columns(4)
    c1.info("**📋 Schema**\nTables, columns, PKs, FKs, types")
    c2.info("**💊 Data Quality**\nNulls, freshness, completeness")
    c3.info("**⭐ Causal AI**\nDirectional influences + Health Scores + YAML contracts")
    c4.info("**🤖 AI Narratives**\nBusiness context via selected AI provider")
    st.markdown("---")
    st.markdown("**Perfect for Olist Brazilian E-Commerce dataset:** customers → orders → order_items → products/sellers.")
    st.stop()
...
# ═══ TAB 2 — ER MAP (Olist-ready) ═══════════════════════════
with t2:
    st.subheader("Entity Relationship Map — Olist Style")
    st.caption("🟠 Hub tables (3+ links) · 🔵 Regular tables · Hover for row counts")
    ...
    with cr:
        ...
        if hubs:
            st.subheader("Most Connected (Hub) Tables")
...
with t4:
    st.markdown("""
    > ⭐ **Causal Intelligence Engine** — Finds *directional* influences (not just correlation)  
    using Mutual Information & Chi‑Squared tests. Auto‑generates **Predictive Health Scores**  
    and **YAML Data Contracts** ready for dbt / Great Expectations.
    """)
    ...
    if causal_rels:
        ...
        st.dataframe(rel_df[["from", "direction", "to", "method", "strength", "p_value", "insight"]],
    ...
    st.markdown("*Ready for Great Expectations · dbt schema tests · pipelines*")
...
with t6:
    ...
    with d2:
        ...
        st.caption("Human‑readable Markdown")
